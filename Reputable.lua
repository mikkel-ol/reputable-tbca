...
-- Table of pending HTML link builds waiting on item info (newer clients may fire GET_ITEM_INFO_RECEIVED early)
Reputable.waitingForItemHTML = Reputable.waitingForItemHTML or {}
local initiated = false;

-- Newer clients moved a lot of AddOn metadata APIs under C_AddOns.
...
-- Safely register events across Classic/TBC variants. Some newer clients hard-error
-- when registering unknown events.
local function SafeRegisterEvent(frame, eventName)
	if not frame or type(eventName) ~= "string" or eventName == "" then return false end
	local ok = pcall(frame.RegisterEvent, frame, eventName)
	return ok
end

local function RegisterEventCompat(frame, preferred, fallback)
	if preferred and SafeRegisterEvent(frame, preferred) then return preferred end
	if fallback and SafeRegisterEvent(frame, fallback) then return fallback end
	return nil
end

-- Quest title lookup API differs slightly between client versions.
local function GetQuestTitleCompat(questID)
	if C_QuestLog then
		...
		if C_QuestLog.GetQuestInfo then
			...
			if type(info) == "table" then
				return info.title or info.name
			end
			return info
		end
	end
...
Reputable.iconFrame.heroic = Reputable.iconFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
...
function Reputable:getQuestStatus( questID, recheck )
	...
	--	if not Reputable.extraQuestInfo[ questID ].checkedThisSession then
		...
		if not Reputable.extraQuestInfo[ questID ].localizedName then
			...
			local name = GetQuestTitleCompat(questID)
			--debug( questID, name )
...
Reputable:SetScript("OnEvent", function (self, event, ...)
	...
	elseif event == "ADDON_LOADED" then
		...
		elseif event == "QUEST_ACCEPTED" then
			-- Newer clients pass (questLogIndex, questID). Older clients only passed (questLogIndex).
			local questIndex, questID = ...
			if not questID and questIndex then
				if C_QuestLog and C_QuestLog.GetInfo then
					local info = C_QuestLog.GetInfo(questIndex)
					questID = info and info.questID or nil
				elseif GetQuestLogTitle then
					questID = select(8, GetQuestLogTitle(questIndex))
				end
			end
			--debug( questID )
			...
			--	debug( GetGossipAvailableQuests() )
				...
				if npcID == "24370" or npcID == "24369" or npcID == "24393" or npcID == "15351" or npcID == "15350" then
					...
					if C_GossipInfo and C_GossipInfo.GetAvailableQuests then
						...
						questName = quests and quests[1] and quests[1].title or nil
					elseif GetGossipAvailableQuests then
						questName = GetGossipAvailableQuests()
					end
...
do
	local function Reputable_HyperlinkHook(a, b, c, d)
		-- Supports both ChatFrame_OnHyperlinkShow(self, link, text, button)
		-- and SetItemRef(link, text, button, chatFrame)
		local link, text, button
		if type(a) == "table" and type(b) == "string" then
			link, text, button = b, c, d
		else
			link, text, button = a, b, c
		end
		...
		if output then
			...
			if editBox and editBox.GetText and editBox.SetText then
				local msg = editBox:GetText()
				if msg then
					editBox:SetText("")
					-- Replace the just-inserted hyperlink with our formatted version
	...
	if type(_G.ChatFrame_OnHyperlinkShow) == "function" then
		hooksecurefunc("ChatFrame_OnHyperlinkShow", Reputable_HyperlinkHook)
	else
		-- Some Classic-era clients don't expose ChatFrame_OnHyperlinkShow globally; SetItemRef is stable.
		hooksecurefunc("SetItemRef", Reputable_HyperlinkHook)
	end
end
...
local function repBarOnEnter( self, ... )
	...
	-- TBC Anniversary: the watched reputation bar may not be created yet, and in some builds
	-- it's managed by StatusTrackingBarManager. Hook lazily and defensively.
	local function Reputable_GetReputationWatchBar()
		-- Classic-style global
		local bar = _G.ReputationWatchBar or _G.ReputationWatchStatusBar
		if bar then return bar end

		-- StatusTrackingBarManager (retail-style backport)
		local mgr = _G.StatusTrackingBarManager
		if mgr then
			-- Common direct member name
			if mgr.ReputationBar then return mgr.ReputationBar end
			-- Scan bar collection if present
			local bars = mgr.bars
			if type(bars) == "table" then
				for _, b in pairs(bars) do
					if b and b.GetName then
						local n = b:GetName() or ""
						if n:find("Reputation", 1, true) then
							return b
						end
					end
				end
	...
	local function Reputable_HookReputationWatchBar()
		local bar = Reputable_GetReputationWatchBar()
		if not bar or bar.__reputable_hooked then return end
		bar.__reputable_hooked = true
		bar:HookScript("OnEnter", function(self) createFactionToolTip(self, self.factionID, nil) end)
		bar:HookScript("OnLeave", function() GameTooltip:Hide() end)
	end

	-- Try immediately, and also after login/entering world in case the bar is created later.
	Reputable_HookReputationWatchBar()
	local repWatchHookFrame = CreateFrame("Frame")
	repWatchHookFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
	repWatchHookFrame:RegisterEvent("PLAYER_LOGIN")
	repWatchHookFrame:SetScript("OnEvent", function()
		Reputable_HookReputationWatchBar()
	end)
...
-- XP bar tooltip hook (TBC Anniversary compatibility)
local function Reputable_GetXPBar()
	if _G.MainMenuExpBar then return _G.MainMenuExpBar end
	if _G.PlayerXPBar then return _G.PlayerXPBar end
	-- Retail/modern-style status tracking containers (some TBC builds use these)
	local container = _G.MainStatusTrackingBarContainer or _G.SecondaryStatusTrackingBarContainer
	if container and container.barFrames then
		for _, bar in ipairs(container.barFrames) do
			local n = bar.GetName and bar:GetName() or ""
			if n:find("Exp") or n:find("XP") then return bar end
		end
	end
	local mgr = _G.StatusTrackingBarManager
	if mgr and mgr.bars then
...
local function Reputable_HookXPBar()
	local bar = Reputable_GetXPBar()
	if not bar or bar.ReputableXPTooltipHooked then return end
	bar.ReputableXPTooltipHooked = true

	bar:HookScript("OnEnter", function(self)
		if Reputable_Data and Reputable_Data.global.xpToolTip then
			if _G.ExhaustionTick then ExhaustionTick.timer = nil end
			GameTooltip:SetOwner(self, "ANCHOR_CURSOR", 0, 20)
...
Reputable_HookXPBar()
local reputableXPHooker = CreateFrame("Frame")
reputableXPHooker:RegisterEvent("PLAYER_LOGIN")
reputableXPHooker:RegisterEvent("PLAYER_ENTERING_WORLD")
reputableXPHooker:SetScript("OnEvent", Reputable_HookXPBar)
...
questRepFrame.text = questRepFrame:CreateFontString(nil, "OVERLAY", "QuestFont")
...
questLogRepFrame.text = questLogRepFrame:CreateFontString(nil, "OVERLAY", "QuestFont")
	...
	if ( initiate or initiated ) then
		...
		for factionIndex = 1, numFactions do
			...
			if isHeader and isCollapsed then
				...
				local thisRepBar = _G["ReputationBar" .. factionIndex]
			if thisRepBar then
				if not thisRepBar.ReputableScriptAdded then
					...
					thisRepBar.title = thisRepBar:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
				...
				if initiate then
					...
					if earnedValue ~= oldValue then
						...
						if Reputable_Data.global.cframeEnabled then
							...
							local chatframe = _G["ChatFrame" .. chatFrameOutput]
							if chatframe then chatframe:AddMessage(newmessage,0.5,0.5,1) end
						end
...
-- LEARNED_SPELL_IN_TAB was removed in newer clients; use LEARNED_SPELL_IN_SKILL_LINE when available.
if not SafeRegisterEvent(Reputable, "LEARNED_SPELL_IN_SKILL_LINE") then
	SafeRegisterEvent(Reputable, "LEARNED_SPELL_IN_TAB")
end
